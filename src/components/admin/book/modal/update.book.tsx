...
interface IProps {
  ...
  setItem: (v: IBookTable | null) => void;
}
...
const ModalUpdateBook = (props: IProps) => {
  const { openModal, setOpenModal, reloadTable, item, setItem } = props;
  ...
  useEffect(() => {
    const fetchImage = async () => {
      if (item) {
        ...
        const arrSlider = item.slider.map((Item: string) => {
          return {
            uid: uuidv4(),
            name: Item,
            status: "done",
  ...
  const onFinish: FormProps<FieldType>["onFinish"] = async (values) => {
    const { mainText, author, price, quantity, category, _id } = values;
